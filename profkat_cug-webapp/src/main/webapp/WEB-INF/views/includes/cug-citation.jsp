...
<div id="citation" class="ir-box ir-box-emph py-2 profkat-citation">
  ...
  <c:choose>
    <c:when test="${fn:contains(mcrid, '_matrikel_')}">
      <c:set var="last"><x:out select="$mcrobj/mycoreobject/metadata/box.name/name/surname" /></c:set>
      <c:set var="first"><x:out select="$mcrobj/mycoreobject/metadata/box.name/name/firstname" /></c:set>
    </c:when>
    <c:otherwise>
