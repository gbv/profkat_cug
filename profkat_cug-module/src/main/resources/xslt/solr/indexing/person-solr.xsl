...
<xsl:stylesheet version="3.0"
  ...
  <xsl:import href="xslImport:solr-document:solr/indexing/person-solr.xsl" />
  ...
  <xsl:template match="mycoreobject[contains(@ID,'_person_')]">
    ...
    <xsl:apply-templates select="metadata" mode="person" />
  ...
  <xsl:template match="metadata" mode="person">
    <xsl:apply-imports/>
    ...
    <field name="profkat.idx_profname.facet">
      <xsl:value-of select="solrutil:get-name-facet($headline)" />
    </field>
